...
export async function transcribe(
  ...
  if (audio.byteLength > MAX_AUDIO_BYTES)
    throw new Error('That recording is too large. Record a shorter take.')
  ...
  if (response.status === 404) {
    ...
    if (fallback)
      return transcribe(
        {
          ...settings,
          voice: { ...settings.voice, inputModel: `${fallback.id}:${fallback.model}` }
        },
        request
      )
    throw new Error(
      `${provider.name} has no transcription endpoint. Configure OpenAI/Groq Whisper or another STT provider under Settings → Voice.`
    )
  }
  if (!response.ok)
    throw new Error(
      `${provider.name} could not transcribe that: ${describe(response.status, body)}`
    )
...
export async function speak(
  ...
  if (response.status === 404)
    throw new Error(
      `${provider.name} has no speech endpoint. Use the computer's voices or configure a TTS provider under Settings → Voice.`
    )
  if (!response.ok)
    throw new Error(
      `${provider.name} could not speak that: ${describe(response.status, await response.text())}`
    )
  return {
...
function findTranscriptionFallback(
  settings: Settings,
...
function resolveVoiceModel(
  ...
  if (!ref)
    throw new Error(
      `No ${direction === 'input' ? 'transcription' : 'speech'} model is chosen. Pick one under Settings → Voice.`
    )
  ...
  if (!provider.enabled || !provider.apiKey)
    throw new Error(`${provider.name} is not configured for voice.`)
  if (provider.kind !== 'openai')
    throw new Error(`${provider.name} does not offer OpenAI-compatible speech endpoints.`)
...
function extensionFor(mediaType: string): string {
  return mediaType.split('/')[1]?.split(';')[0] || 'webm'
}
