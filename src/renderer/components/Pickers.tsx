...
export function QuickOpen({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    void window.forge.index
      .files()
      .then(setFiles)
      .catch((error) => {
        useStore.getState().pushError(`Could not index files: ${(error as Error).message}`)
        setFiles([])
      })
  }, [])
  ...
  const items = useMemo<PaletteItem[]>(
    () =>
      files.map((file) => ({
        ...
        label: file,
        run: async () => {
          const content = await window.forge.workspace.read(file).catch((error) => {
            useStore.getState().pushError(`Could not open ${file}: ${(error as Error).message}`)
            return null
          })
  ...
  return useMemo<PaletteItem[]>(() => {
    ...
    const commands: PaletteItem[] = [
      ...
      { id: 'go-file', label: t('Go to file'), hint: 'Ctrl+P', run: ui({ picker: 'files' }) },
      {
        ...
        hint: 'Ctrl+Shift+O',
        run: ui({ picker: 'symbols' })
      },
      {
        id: 'search',
        label: t('Search across files'),
        hint: 'Ctrl+Shift+F',
        run: ui({ sidePanel: 'search', sidebarWidth: 320 })
      },

      {
        id: 'mode-chat',
        label: t('Switch to Chat'),
        hint: 'Ctrl+1',
        run: () => void store.saveSettings({ mode: 'chat' })
      },
      {
        id: 'mode-edit',
        label: t('Switch to Work'),
        hint: 'Ctrl+2',
        run: () => void store.saveSettings({ mode: 'agent' })
      },

      {
        ...
        label: t('Dashboard'),
        run: () => {
          void store.saveSettings({ mode: 'chat' })
          store.patchUi({ chatPane: 'dashboard' })
        }
      },
      {
        id: 'pane-tasks',
      ...
      },
      {
        ...
        run: () => {
          void store.saveSettings({ mode: 'chat' })
          store.patchUi({ chatPane: 'browser' })
        }
      },
      {
        id: 'pane-live',
      ...
      },
      {
        ...
        run: () => {
          void store.saveSettings({ mode: 'chat' })
          store.patchUi({ chatPane: 'checkpoints' })
        }
      },

      {
        ...
        hint: 'Ctrl+`',
        run: () => store.patchUi({ terminalOpen: !store.ui.terminalOpen })
      },
      {
        id: 'sidebar',
        label: t('Toggle sidebar'),
        hint: 'Ctrl+B',
        run: () => store.patchUi({ sidebarWidth: store.ui.sidebarWidth > 0 ? 0 : 240 })
      },
      {
        id: 'review',
        label: t('Review changes'),
        hint: 'Ctrl+Shift+R',
        run: ui({ mainView: 'review' })
      },
      {
        id: 'git',
        label: t('Source control'),
        hint: 'Ctrl+Shift+G',
        run: ui({ sidePanel: 'git', sidebarWidth: 260 })
      },

      {
        id: 'export',
        label: t('Export this conversation to Markdown'),
        run: () => void store.exportConversation()
      },

      { id: 'settings', label: t('Settings'), hint: 'Ctrl+,', run: ui({ settingsOpen: true }) },
      {
        id: 'settings-providers',
        label: t('Settings: Providers & models'),
        run: ui({ settingsOpen: true, settingsSection: 'providers' })
      },
      {
        id: 'settings-voice',
        label: t('Settings: Voice'),
        run: ui({ settingsOpen: true, settingsSection: 'voice' })
      },
      {
        id: 'settings-appearance',
        label: t('Settings: Appearance'),
        run: ui({ settingsOpen: true, settingsSection: 'appearance' })
      },
      {
        id: 'settings-prompts',
        label: t('Settings: Prompt library'),
        run: ui({ settingsOpen: true, settingsSection: 'prompts' })
      },
      {
        id: 'settings-permissions',
        label: t('Settings: Permissions'),
        run: ui({ settingsOpen: true, settingsSection: 'permissions' })
      },

      {
        id: 'read-only',
        label: t('Read-only: look but do not touch'),
        run: () => void store.saveSettings({ readOnly: true, bypassPermissions: false })
      },
      {
        id: 'unlock',
        label: t('Allow edits again'),
        run: () => void store.saveSettings({ readOnly: false })
      },
      {
        id: 'bypass',
        label: t('Approve everything without asking'),
        run: () => void store.saveSettings({ bypassPermissions: true, readOnly: false })
      }
    ]
